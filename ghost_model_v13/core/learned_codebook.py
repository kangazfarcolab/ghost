...
class CodebookLinear(nn.Module):
    ...
    def __call__(self, x):
        ...
        # Combine: sign * magnitude (USE CLAMPED WEIGHTS!)
        magnitudes = mx.abs(w_clamped) * s  # FIXED: was using self.weight (unclamped)
        w_quantized = signs * magnitudes
        
        # Straight-Through Estimator for training
        # The gradient flows through self.weight, but forward uses clamped version
        w_ste = mx.stop_gradient(w_quantized - w_clamped) + w_clamped
